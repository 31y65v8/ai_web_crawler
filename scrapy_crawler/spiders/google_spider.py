...
'''
使用了scrapyapi地址代理，爬取json格式响应数据（标题、url、摘要、爬取时间）
'''
...
class GoogleSpiderSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 创建数据表
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS ch_webpages (
    ...
    def start_requests(self):
        ...
        # 确保查询生成了正确的组合
        self.logger.debug(f"生成的搜索关键词: {ch_combined_queries}")
        ...
        for i in range(0, len(ch_combined_queries), batch_size):
            ...
            for query in batch_queries:
                self.logger.debug(f"正在处理查询: {query}")# 打印当前正在处理的查询
                url = create_google_url(query)
                yield scrapy.Request(get_url(url), callback=self.parse, meta={'pos': 0})
                '''
                 Scrapy 中，yield 关键词的作用是生成一个请求（scrapy.Request）并将其返回给 Scrapy 引擎，而不是像普通函数一样直接返回结果。
                 使用 yield 的好处是它允许爬虫在处理每个请求时不会阻塞，而是会继续执行其他任务，直到请求完成或回调函数被触发。
                 callback=self.parse:当请求响应返回时scrapy调用self.parse方法处理响应
                 response.meta 是 Scrapy 请求的一个字典，用于在请求和响应之间传递数据。
                '''
    ...
    def parse_article(self, response):
        ...
        file_name = f'html_files/{title}_{pos}.html'.replace(" ", "_")  # 去掉空格以便命名

        if os.path.exists(file_name):
            self.logger.info(f"HTML file saved at: {file_name}")
        else:
            self.logger.error(f"Failed to save HTML file at: {file_name}")
        ...
        # 将数据存储到数据库中
        try:
            self.cursor.execute('''INSERT INTO ch_webpages (title, html_file_path, link, position, date)
    ...
    def parse(self, response):
        ...
        if response.status != 200:
            self.logger.error(f"请求失败，状态码: {response.status} for URL: {response.url}")
            return  # 如果请求失败，跳过

        try:
            di = json.loads(response.text)
            self.logger.debug(f"收到JSON数据: {json.dumps(di, indent=2)}")  # 打印 JSON 内容
        except json.JSONDecodeError:
            self.logger.error("解析失败，无法将响应解析为python字典")
            return
        pos = response.meta['pos']#第几条结果
        dt = datetime.now().strftime('%Y-%m-%d %H:%M:%S')#爬取时间
